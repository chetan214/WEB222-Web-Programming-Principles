...
function toCamelCase(name, uppercase) {
  ...
  if (uppercase) {
    camelCase = words[0].slice(0, 1).toUpperCase();
    camelCase += words[0].slice(1);
    //console.log(ret);
  } else {
    camelCase = words[0];
  }
  ...
  for (let i = 1; i < words.length; i++) {
    camelCase += words[i].slice(0, 1).toUpperCase();
    camelCase += words[i].slice(1);
  }
  return camelCase;
}
/*******************************************************************************
...
function createLinkTag(rel, href) {
  // Remove leading and trailing whitespace from the rel and href values
  rel = rel.trim();
  href = href.trim();

  // Check if the rel or href values contain any special characters
  const relHasSpecialChar = /[ "'`<>\t]/g.test(rel);
  const hrefHasSpecialChar = /[ "'`<>\t]/g.test(href);

  // Construct the <link> tag string
  let linkTag = `<link${relHasSpecialChar ? ` rel="${rel}"` : ` rel=${rel}`}`;
  linkTag += `${hrefHasSpecialChar ? ` href="${href}"` : ` href=${href}`}>`;
...
function parseDateString(value) {
  ...
  if (matches[1]) {
    ...
    const month = parseInt(matches[5]) - 1;
    const year = parseInt(matches[6]);
    date.setFullYear(year, month, day);
  }
...
function toDateString(value, format) {
  ...
  // Format the date string according to the supplied format
  let dateString = '';
  switch (format) {
    case 'YYYY/MM/DD':
...
function formatTimes(...values) {
  // Normalize each time string using the normalizeTime() function
  const normalizedTimes = values.map(normalizeTime);

  // Filter out any null values
  const validTimes = normalizedTimes.filter((time) => time !== null);

  // Combine the remaining elements into a string, separated by commas and spaces
